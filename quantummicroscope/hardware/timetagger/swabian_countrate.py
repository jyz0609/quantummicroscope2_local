...
def _require_timetagger():
    if TimeTagger is None:
        raise RuntimeError("Swabian TimeTagger SDK is not installed in this Python environment.")
    return TimeTagger
...
class run_swabian:
    ...
    def __init__(self,filepath=""):
        ...
        self.dump = TT.Dump(tagger=self.tagger, filename=self.swabian_file, max_tags=-1,
    ...
    def free(self):
        _require_timetagger().freeTimeTagger(self.tagger)
    ...
    def get_countrate(self,channels = [2, 3]):
        ...
        counter = _require_timetagger().Countrate(self.tagger, channels)
        time.sleep(3)
...
if __name__ == "__main__":
    ...
    filepath = "K:/Microscope/Data/250310/infotest_date(250310)_time(15h57m14s)_scantime(200.0)_dwellTime(0.005)_xAmp(0.31949)_yAmp(0.31949)_xyDim(200).timeres"
    tagger = run_swabian(filepath=filepath)
    count = _require_timetagger().Countrate(tagger.tagger,[2,3])
    time.sleep(2)
    countratearr= count.getData()
    print(f"countrate = {count.getData()}")
    print(type(countratearr))
    _require_timetagger().freeTimeTagger(tagger.tagger)
