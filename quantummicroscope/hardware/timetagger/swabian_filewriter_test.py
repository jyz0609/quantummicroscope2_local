...
def write_file(filename):
    ...
    fw = TT.FileWriter(tagger, filename, [1,2])

    counter = TT.Counter(tagger=tagger, channels=[1, 2], binwidth=int(1e9), n_values=10000)
...
def read_file(filename):
    time_before = time.time()
    TT = _require_timetagger()
    tagger = TT.createTimeTaggerVirtual(filename=filename)
    counter = TT.Counter(tagger=tagger, channels=[1, 2], binwidth=int(1e9), n_values=10000)
    tagger.run()
    is_running = True
    fig = plt.figure()
